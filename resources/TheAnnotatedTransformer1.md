...
```
#### 残差接続の計算
上記では、同一層から伝播されてきた情報と、他のレイヤーの情報をただ単に足し合わせるのではなく、残差接続(Residual Connection)[[4]][res]と呼ばれる手法を用いてそれらを合成処理することによって、深いネットワークでの勾配消失を防ぎ、情報が層を越えて伝播しやすくなる。と書きました。<br>
[res]:https://arxiv.org/abs/1512.03385
さらに、ドロップアウト[[6]][drop]という手法があります。これは過学習を防ぐために使用され、学習時にニューロンをランダムに無効化することによって、モデルの汎化能力を高めます。<br>
[drop]:https://jmlr.org/papers/v15/srivastava14a.html
